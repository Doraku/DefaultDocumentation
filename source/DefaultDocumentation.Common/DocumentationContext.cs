...
{
    internal sealed class DocumentationContext
    {
        ...
        public DocumentationContext(Settings settings, SectionWriter[] sectionWriters, IReadOnlyDictionary<string, ElementWriter> elementWriters)
            : this(settings, sectionWriters, elementWriters, DocItemReader.GetItems(settings))
        { }

        public string GetFileName(DocItem item) => _fileNames.GetOrAdd(item, i => Settings.PathCleaner.Clean(i is AssemblyDocItem ? i.FullName : Settings.FileNameMode switch
        {
        ...
        public string GetUrl(string id) => TryGetDocItem(id, out DocItem item) ? GetUrl(item) : _urls.GetOrAdd(id, i => i.ToDotNetApiUrl());
