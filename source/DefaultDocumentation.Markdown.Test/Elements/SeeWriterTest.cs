...
{
    public sealed class SeeWriterTest : AElementWriterTest<SeeWriter>
    {
        private readonly DocItem _item;

        public SeeWriterTest()
        {
            _item = new ClassDocItem(null, AssemblyInfo.Get<ITypeDefinition>($"T:{typeof(SeeWriterTest).FullName}"), null);
        }
        ...
        public void Write_should_write_When_cref() => Test(
            ...
            $"[{_item.Name}](DefaultDocumentation_Markdown_Elements_SeeWriterTest.md '{_item.FullName}')");

        [Fact]
        public void Write_should_write_When_cref_with_value() => Test(
            _item,
            new XElement("see", new XAttribute("cref", _item.Id), "dummy"),
            $"[dummy](DefaultDocumentation_Markdown_Elements_SeeWriterTest.md '{_item.FullName}')");
        [Fact]
